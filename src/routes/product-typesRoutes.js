...
const { deleteProductType, updateProductType, createProductType, getProductType } = require('../database/product-types');

router.get('/', async (apiRequest, apiResponse) => {
    apiResponse.send(await getProductType());
});
...
router.post('/', async (apiRequest, apiResponse) => {
    ...
    apiResponse.send({
        message: 'New new product type created.',
        allProductType: await getProductType(),
        thanks: true
    });
});
